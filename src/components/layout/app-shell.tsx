...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex w-full overflow-hidden">
      ...
      <div className="flex min-w-0 flex-1 flex-col bg-background border-b h-screen">
        ...
        <div className="flex min-h-0 flex-1 w-full">
          <aside className="hidden h-full lg:flex w-[350px] shrink-0 border-r border-sidebar-border">
            <Suspense>
              <SurahSidebar currentId={surahId} />
            </Suspense>
          </aside>
      ...
      <Sheet open={navOpen} onOpenChange={setNavOpen}>
        <SheetContent side="left" className="p-0 w-[320px] bg-sidebar border-sidebar-border">
          <SurahSidebar currentId={surahId} onNavigate={() => setNavOpen(false)} />
        </SheetContent>
      </Sheet>
